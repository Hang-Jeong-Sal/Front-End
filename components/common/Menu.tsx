...
const CategoryFilter = ({
  ...
  currentCategories: GroundCategory[];
  onSelectCallback: (selectedCategories: GroundCategory[]) => void;
}) => {
  ...
  const [filterCategories, setFilterCategories] =
    useState<GroundCategory[]>(currentCategories);
  ...
  return (
    <ThemeProvider theme={MainTheme}>
      <VerticalContentContainer
        style={{ height: 'fit-content', alignItems: 'normal' }}
      >
        <MenuItem
          value={'전체'}
          onClick={() => {
        ...
        {GroundCategories.map((category: GroundCategory, idx: number) => {
          return (
            <MenuItem
              ...
              onClick={() => {
                if (filterCategories.includes(category)) {
                  ...
                  setFilterCategories([
                    ...getSplicedArray(filterCategories, idx, 1),
                  ]);
                } else setFilterCategories([...filterCategories, category]);
              }}
              sx={{
                height: '16px',
              }}
            >
              ...
              <ListItemText
                primary={GroundCategory_ko[category]}
                sx={{ width: '80vw' }}
              />
            </MenuItem>
          );
...
const RadiusFilter = ({
  ...
  onChangeCallback: (changedRadius: number) => void;
}) => {
    ...
    <ThemeProvider theme={MainTheme}>
      ...
      >
        ...
        <Slider
          ...
          min={DefaultGroundSearchOption['area'].from}
          max={DefaultGroundSearchOption['area'].to}
          size={'medium'}
          valueLabelDisplay="auto"
          sx={{
            width: 'calc(100vw - 40px)',
          ...
          onChange={(e, v) => {
            if (Array.isArray(v)) onChangeCallback({ from: v[0], to: v[1] });
          }}
        />
      </VerticalContentContainer>
    </ThemeProvider>
  ...
  return (
    <ThemeProvider theme={MainTheme}>
      ...
      >
        ...
        <Slider
          value={[defaultValue.from, defaultValue.to]}
          min={DefaultGroundSearchOption['price'].from}
          max={DefaultGroundSearchOption['price'].to}
          size={'medium'}
          valueLabelDisplay="auto"
          sx={{
            width: 'calc(100vw - 40px)',
...
const ConvenientFilter = ({
  ...
  currentConvenients: GroundConvenient[];
  onSelectCallback: (selectedConvenients: GroundConvenient[]) => void;
}) => {
  const getSplicedArray = (
    arr: any[],
    start: number,
  ...
  const [filterConvenients, setFilterConnenients] =
    useState<GroundConvenient[]>(currentConvenients);
        ...
        {GroundConvenients.map((convenient: GroundConvenient, idx: number) => {
          return (
            <MenuItem
              ...
              onClick={() => {
                if (filterConvenients.includes(convenient)) {
                  ...
                  setFilterConnenients([
                    ...getSplicedArray(filterConvenients, idx, 1),
                  ]);
                } else setFilterConnenients([...filterConvenients, convenient]);
              }}
              sx={{
                height: '16px',
              }}
            >
              ...
              <ListItemText
                primary={GroundConvenient_ko[convenient]}
                sx={{ width: '80vw' }}
              />
            </MenuItem>
          );
...
const PeriodFilter = ({
  ...
  currentPeriod: { from: Dayjs; to: Dayjs };
  onChangeCallback: (changedPeriod: { from: Dayjs; to: Dayjs }) => void;
}) => {
  const [filterPeriod, setFilterPeriod] = useState<{ from: Dayjs; to: Dayjs }>(
    currentPeriod
  );
  ...
  return (
    ...
    >
      <div
        style={{ fontWeight: '700', fontSize: '28px', margin: '0 0 20px 20px' }}
      >
        {'시작'}
      </div>
...
export const SearchDrawer = ({
  ...
  onChangeFilterOptionsCallback,
}: {
  options: SearchOption[];
  currentOption: SearchOption;
  currentFilterOptions: GroundSearchOption;
  onOptionClickCallback: (clickedOption: SearchOption) => void;
  onCloseCallback: (isClose: boolean) => void;
  onChangeFilterOptionsCallback: (
    changedFilterOptions: GroundSearchOption
  ) => void;
}) => {
  const [selectedFilterOptions, setSelectedFilterOptions] =
    useState<GroundSearchOption>(currentFilterOptions);
    ...
    convenient: (
      <ConvenientFilter
        ...
        onSelectCallback={(selectedConvenients: GroundConvenient[]) => {
          setSelectedFilterOptions({
            ...selectedFilterOptions,
            convenient: selectedConvenients,
          });
        }}
      />
    ),
  ...
  return (
    ...
    >
      <HorizontalContentContainer>
        <HorizontalScrollableContentContainer
          style={{ marginTop: '28px', paddingBottom: '14px' }}
        >
                ...
                style={{
                  ...
                  color:
                    currentOption === option
                      ? 'black'
                      : MainTheme.palette.info.contrastText,
                }}
                onClick={() => {
                  onOptionClickCallback(option);
                }}
              >
      ...
      <HorizontalContentContainer
        style={{
          ...
          borderTop: `1px solid ${MainTheme.palette.info.contrastText}`,
            ...
            onClick={() => {
              setSelectedFilterOptions(
                (prev) =>
                  (prev = {
                    ...selectedFilterOptions,
                    [currentOption]: DefaultGroundSearchOption[currentOption],
                  })
              );
            }}
          >
